...
| 2026-06-04 | DevOps | Commit y push de migración completa a Supabase + GitHub Pages: 11 archivos (`.gitignore`, `developer_context.md`, `index.html`, `js/app.js`, `server.py`, `ANALISIS_REGLAS_NEGOCIO.md`, `deploy_oracle.md`, `js/supabase-adapter.js`, `migrate.py`, `migrate_to_supabase.py`, `supabase_schema.sql`). Conflicto en `.gitignore` resuelto preservando exclusiones de `database.db/json`. |
| 2026-06-04 | Fix | `migrate_to_supabase.py`: reemplazada anon key por placeholder `SERVICE_ROLE_KEY` (la clave real no se commitea), corregido DELETE de tabla `config` (usa PK `key`, no `id`). Migración pendiente de ejecutar con service_role key real. |
